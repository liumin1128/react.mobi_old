...
function DynamicEditor({ initialValues, loading, onSubmit }) {
  const input = useRef()
        ...
        <Box display='flex' flexGrow={1}>
          ...
          >
            <ButtonBase style={{ color: '#999' }} aria-label='Camera'>
              <VideoIcon color={iframe ? 'primary' : 'inherit'} style={{ width: 20, height: 20 }} />
            </ButtonBase>
          </Popper>
...
DynamicEditor.defaultProps = {
  initialValues: { content: '', pictures: [], iframe: undefined },
  onSubmit: () => {},
  loading: false,
}
